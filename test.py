...
SYSTEM_PROMPT = """You are a helpful voice assistant who responds briefly in a clear and concise manner."""
...
class TextToSpeech:
    def __init__(self, model_name: str, sounds_folder: str="disfluencies") -> None:
        ...
        self.initial_responses = [
            "init_1.mp3", "init_2.mp3", "init_3.mp3", "init_4.mp3", 
            "init_5.mp3", "init_6.mp3", "init_7.mp3", "init_8.mp3"
        ]
    ...
    def __load_intro__(self, folder: str):
        self.intro_sounds = []
        for filename in self.initial_responses:
            path = os.path.join(folder, filename)
            if os.path.isfile(path):
                self.intro_sounds.append(pygame.mixer.Sound(path))
            else:
    ...
    def play_initial_response(self):
        sound = random.choice(self.intro_sounds)
        sound.play()
...
class ConversationManager:
    def __init__(self):
        ...
        self.tts = TextToSpeech(tts_model)
...
if __name__ == "__main__":
    ...
    async def main():
        while True:
            if len(manager.llm.messages)==1:
                manager.tts.play_initial_response()
            query = input("Enter your query: ")
            if query:
                manager.tts.play_disfluent_filler()
                response, llm_elapsed_time = await manager.llm.generate(query, True)
                tts_elapsed_time = manager.tts.speak(response, True)
